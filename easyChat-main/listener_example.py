...
def test_wx_listen():
    ...
    print("=== 添加初始监听对象 ===")
    # 使用默认消息处理函数
    listener_manager.add_listener("沈圳")
    ...
    # 启动一个线程用于动态添加监听对象
    dynamic_add_thread = threading.Thread(target=dynamic_add_listener,
                                          args=(listener_manager, ))
    ...
    # 再次启动一个线程用于动态添加监听对象
    dynamic_add_thread2 = threading.Thread(target=dynamic_add_listener,
                                           args=(listener_manager, ))
